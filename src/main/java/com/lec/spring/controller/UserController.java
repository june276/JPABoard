...
public class UserController {
	...
	public String registerOk(
		...
		// 검증에러가 있었다면 redirect 한다.
		if(result.hasErrors()){
			redirectAttribute.addFlashAttribute("username", user.getUsername());
			redirectAttribute.addFlashAttribute("name", user.getName());
			
			List<FieldError> errList = result.getFieldErrors();
			for(FieldError err : errList) {
				redirectAttribute.addFlashAttribute("error", err.getCode()); // 가장 처음에 발견된 에러를 담아ㅣ 보낸다
				break;
			}
